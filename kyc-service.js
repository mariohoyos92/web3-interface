...
function getBearerToken() {
    return new Promise((resolve, reject) => {
        ...
        const r = request.post(tokenOptions, (err, resp, body) => !err ? resolve(body.access) : reject(err));
    });
}
        ...
        return getBearerToken().then((token) => {
            ...
            request.get(business_uri, { headers }, function (err, resp, body) {
                if (!err) {
                    ...
                    const access_codes_uri = business_uri + business_id + '/access-codes/';

                    request.get(access_codes_uri, { headers }, function (err, resp, body) {
                        if (!err) {
                            const codes = JSON.parse(body).results.filter(code => code.is_active);
                            resolve(codes);
                        } else {
                            reject(err);
                        }
